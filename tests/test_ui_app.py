...
class UIAppTest(unittest.TestCase):
    ...
    def test_dashboard_is_available_without_login(self):
        ...
        with tmp:
            ...
            self.assertIn("Local operator", response.text)

    def test_api_status_is_available_without_login(self):
        client, tmp = self._build_client()
        with tmp:
    ...
    def test_dashboard_shows_bot_status_and_note_without_login(self):
        client, tmp = self._build_client()
        with tmp:
            response = client.get("/")
            self.assertEqual(response.status_code, 200)
    ...
    def test_control_endpoint_requires_csrf(self):
        client, tmp = self._build_client()
        with tmp:
    ...
    def test_analyst_api_is_available_without_login(self):
        client, tmp = self._build_client()
        with tmp:
